...
def VLSTMModelTraining(model, model_name, pair, tracks, labels, BATCH_SIZE=32, NB_EPOCHS=100, NB_SAMPLES=50000, VALIDATION_SPLIT=0.2, LR=0.001, pair_reinforce=False):

    full_size = len(labels)
    train_size = full_size*(1-VALIDATION_SPLIT) 
    ...
    for epochs in range(NB_EPOCHS):
        ...
        pair_valid_use = pair_valid[EindexValid][:NB_SAMPLES*VALIDATION_SPLIT]
        tracks_valid_use = tracks_valid[EindexValid][:NB_SAMPLES*VALIDATION_SPLIT]
        lables_valid_use = labelss_valid[EindexValid][:NB_SAMPLES*VALIDATION_SPLIT]
