...
if __name__ == "__main__":
    ...
    model_name = "Pair_Model_Standard_cc_03-04_bb_04-05_201124_2"
    ...
    #loss_weights = [1.0, 0.0] # vertex, position
    ...
    model_name = model_name + "_loss_weights_" + str(0.1) + "_" + str(0.9) + "_1000epochs"
    ...
    model_name = model_name + "_plus_loss_weights_0.9_0.1_1500epochs"
    modeltools.SavePairHistory(history, model_name)
    modeltools.SavePairModel(model, model_name)

    model, history = training.PairModelTraining(model, model_name, variables, vertex, position, 
                                                BATCH_SIZE=batch_size, NB_EPOCHS=500, VALIDATION_SPLIT=validation_split, LR=lr/10,
                                                Custom_Weights=custom_weights, Loss_Weights=[0.5, 0.5])
    model_name = model_name + "_plus_loss_weights_0.5_0.5_500epochs_lr_0.0001"
    modeltools.SavePairHistory(history, model_name)
    modeltools.SavePairModel(model, model_name)
