...
if __name__ == "__main__":
    ...
    MCBottom_RecoSV = 0
    MCBottom_RecoSV_samechain = 0
    MCBottom_RecoSV_sameparticle = 0
    MCCharm_RecoSV = 0
    MCCharm_RecoSV_samechain = 0
    MCCharm_RecoSV_sameparticle = 0
    ...
    pair_pos_model_name = "Pair_Pos_Model_vfdnn04_1Msamples_2500epochs"

    lstm_model_name = "Attention_Bidirectional_VLSTM_Model_vfdnn06_50000samples_100epochs"
    slstm_model_name = "Attention_Bidirectional_VLSTM_Model_vfdnn06_50000samples_100epochs_ps_100epochs_s"
    ...
    variables = data[:1000000, 3:47]
    pair_model = MODELBANK.load_model(pair_model_name)
    pair_pos_model = MODELBANK.load_model(pair_pos_model_name)
    print("Pair Model load successfully")
        
    pair_model.compile(loss='categorical_crossentropy', 
                       optimizer=keras.optimizers.Adam(lr=0.001), 
                       metrics=['accuracy'])
    pair_pos_model.compile(loss='mean_squared_logarithmic_error',
                           optimizer=keras.optimizers.Adam(lr=0.001), 
                           metrics=['mae'])
    ...
    predict_pos = pair_pos_model.predict([variables])
    predict_pos = np.array(predict_pos, dtype=float)

    data = np.concatenate([data[:1000000], predict_vertex_finder], 1)
    ori_data = np.concatenate([ori_data[:1000000], predict_pos], 1)
    ...
    for event_num in event_nums:
        ...
        event_oridata = [datum for datum in ori_data if datum[0]==event_num]
        ...
        # Zero matrix ture bc
        vertex_mat_tcc = [[0 for j in range(int(track_num))] for i in range(int(track_num))] # Max number of tracks
        vertex_mat_tbb = [[0 for j in range(int(track_num))] for i in range(int(track_num))] # Max number of tracks
        ...
        for t in range(int(track_num)):
            vertex_mat_tcc[t][t] = 1
            vertex_mat_tbb[t][t] = 1
        ...
        # ================================================================================================= #
            ...
            # True secondary vertex ver3
                ...
                vertex_mat_tcc[int(event_datum[1])][int(event_datum[2])] = 1
                vertex_mat_tcc[int(event_datum[2])][int(event_datum[1])] = 1
            elif event_datum[57] == 3:
                true_bbvertex_list_index.append([int(event_datum[1]), int(event_datum[2])])
                vertex_mat_tbb[int(event_datum[1])][int(event_datum[2])] = 1
                vertex_mat_tbb[int(event_datum[2])][int(event_datum[1])] = 1
        ...
        vertex_mat_tcc = np.array(vertex_mat_tcc)
        vertex_mat_tbb = np.array(vertex_mat_tbb)
        ...
        for t in range(int(track_num)):
            ...
            true_cc_list = [i for i, x in enumerate(vertex_mat_tcc[t, :]) if x == 1]
            if true_cc_list not in true_ccbb_lists and len(true_cc_list) != 1:
                true_ccbb_lists.append(true_cc_list)
            true_bb_list = [i for i, x in enumerate(vertex_mat_tbb[t, :]) if x == 1]
            if true_bb_list not in true_ccbb_lists and len(true_bb_list) != 1:
                true_ccbb_lists.append(true_bb_list)
        ...
        for i, true_ccbb_list in enumerate(true_ccbb_lists):
            for true_ccbb in true_ccbb_list:
                chain_list[int(true_ccbb)] = -(i+1)
        ...
        secondary_index = []
        three_probabilities = []
        for event_datum, pred_label, event_oridatum in zip(event_data, pred_labels, event_oridata):
            #if (pred_label != 2 and pred_label != 3 and pred_label != 4) or event_oridatum[47]>100: # this event datum is not "predicted secondary vertex"
            #if (pred_label != 2 and pred_label != 3 and pred_label != 4) or event_oridatum[47]>10: # this event datum is not "predicted secondary vertex"
            if (pred_label != 2 and pred_label != 3 and pred_label != 4) or event_oridatum[-1]>5: # this event datum is not "predicted secondary vertex"
                continue
            else:
                three_probability = event_datum[-1] + event_datum[-2] + event_datum[-3]
                if three_probability < psvth: continue
                if event_datum[-1] > 0.6 or event_datum[-2] > 0.6: continue
                three_probabilities.append(three_probability)
                print("Predicted Position : " + str(event_oridatum[-1]))
        ...
        mcbcsps = []
        ccbb_count = 0
        for ccbb in range(len(true_ccbb_lists)):
            if -(ccbb+1) in chain_list:
                ccbb_count = ccbb_count + 1
                tcc = [i for i, (x, c) in enumerate(zip(ppccbb_list, chain_list)) if x == "c" and c == -(ccbb+1)]
                tbb = [i for i, (x, c) in enumerate(zip(ppccbb_list, chain_list)) if x == "b" and c == -(ccbb+1)]
                if len(tcc) != 0:
                    print(pycolor.CYAN + "True Secondary Vertex Alone Charm " + str(ccbb_count) + pycolor.END)
                    print(pycolor.CYAN + "cc : " + str(tcc) + pycolor.END)
                    mcbcscs.append(tcc)
                    mcbcsps.append(tcc)
                if len(tbb) != 0:
                    print(pycolor.CYAN + "True Secondary Vertex Alone Bottom " + str(ccbb_count) + pycolor.END)
        ...
        for bc in range(len(true_bc_lists)):
            ...
            mcbcsc = [i for i, c in enumerate(chain_list) if c == bc+1]
            mcbcscs.append(mcbcsc)
            if len(tcc) != 0: mcbcsps.append(tcc)
            if len(tbb) != 0: mcbcsps.append(tbb)
            if len(one) != 0: mcbcsps.append(one)
        ...
        mcp = [i for i, x in enumerate(ppccbb_list) if x == "p"]
        mco = [i for i, (x, c) in enumerate(zip(ppccbb_list, chain_list)) if x == "o" and c == 0]
        ...
        chain_recos = copy.deepcopy(recos)
        particle_recos = copy.deepcopy(recos)
        for vertex in vertices:
            if len(vertex) == 0: continue
            chain_torf = []
            particle_torf = []
            for mcbcsc in mcbcscs:
                chain_torf.append(PRODTOOLS.yinx(vertex, mcbcsc))
            if not any(chain_torf): chain_recos = PRODTOOLS.listremove(vertex, chain_recos)
            for mcbcsp in mcbcsps:
                particle_torf.append(PRODTOOLS.yinx(vertex, mcbcsp))
            if not any(particle_torf): particle_recos = PRODTOOLS.listremove(vertex, particle_recos)

        if len(mcp) != 0:
            for _mcp in mcp:
                if _mcp in recos: mcprecos = mcprecos + 1
        if len(mco) != 0:
            for _mco in mco:
                if _mco in recos: mcorecos = mcorecos + 1
        if len(mcb) != 0:
            for _mcb in mcb:
                if _mcb in recos: mcbrecos = mcbrecos + 1
                if _mcb in chain_recos: mcbrecosc = mcbrecosc + 1
                if _mcb in particle_recos: mcbrecosp = mcbrecosp + 1
        if len(mcc) != 0:
            for _mcc in mcc:
                if _mcc in recos: mccrecos = mccrecos + 1
                if _mcc in chain_recos: mccrecosc = mccrecosc + 1
                if _mcc in particle_recos: mccrecosp = mccrecosp + 1


        print(pycolor.ACCENT + "-------------------------------------------------------------------------------------------------" + pycolor.END)
        if len(mcp) != 0:
            print(pycolor.RED + "MC Primary / Reco SV : " + str(mcprecos/len(mcp)) + pycolor.END)
            pvevent_num = pvevent_num + 1
            MCPrimary_RecoSV = MCPrimary_RecoSV + mcprecos/len(mcp)
        else: print(pycolor.RED + "MC Primary / Reco SV : [not exists]" + pycolor.END)
        if len(mco) != 0:
            print(pycolor.RED + "MC Others  / Reco SV : " + str(mcorecos/len(mco)) + pycolor.END)
            othersevent_num = othersevent_num + 1
            MCOthers_RecoSV = MCOthers_RecoSV + mcorecos/len(mco)
        else: print(pycolor.RED + "MC Others  / Reco SV : [not exists]" + pycolor.END)
        if len(mcb) != 0:
            print(pycolor.RED + "MC Bottom  / Reco SV : " + str(mcbrecos/len(mcb)) \
                              + " Same Chain : " + str(mcbrecosc/len(mcb)) + " Same Particle : " + str(mcbrecosp/len(mcb)) + pycolor.END)
            bbevent_num = bbevent_num + 1
            MCBottom_RecoSV = MCBottom_RecoSV + mcbrecos/len(mcb)
            MCBottom_RecoSV_samechain = MCBottom_RecoSV_samechain + mcbrecosc/len(mcb)
            MCBottom_RecoSV_sameparticle = MCBottom_RecoSV_sameparticle + mcbrecosp/len(mcb)
        else: print(pycolor.RED + "MC Bottom  / Reco SV : [not exists] Same Chain : [not exists] Same Particle : [not exists]" + pycolor.END)
        if len(mcc) != 0:
            print(pycolor.RED + "MC Charm   / Reco SV : " + str(mccrecos/len(mcc)) \
                              + " Same Chain : " + str(mccrecosc/len(mcc)) + " Same Particle : " + str(mccrecosp/len(mcc)) + pycolor.END)
            ccevent_num = ccevent_num + 1
            MCCharm_RecoSV = MCCharm_RecoSV + mccrecos/len(mcc)
            MCCharm_RecoSV_samechain = MCCharm_RecoSV_samechain + mccrecosc/len(mcc)
            MCCharm_RecoSV_sameparticle = MCCharm_RecoSV_sameparticle + mccrecosp/len(mcc)
        else: print(pycolor.RED + "MC Charm   / Reco SV : [not exists] Same Chain : [not exists] Same Particle : [not exists]" + pycolor.END)
        print(pycolor.ACCENT + "-------------------------------------------------------------------------------------------------" + pycolor.END)


    print(pycolor.RED + "MC Primary / Reco SV : " + str(MCPrimary_RecoSV/pvevent_num) + pycolor.END)
    print(pycolor.RED + "MC Others  / Reco SV : " + str(MCOthers_RecoSV/othersevent_num) + pycolor.END)
    print(pycolor.RED + "MC Bottom  / Reco SV : " + str(MCBottom_RecoSV/bbevent_num) \
                      + " Same Chain : " + str(MCBottom_RecoSV_samechain/bbevent_num) + " Same Particle : " + str(MCBottom_RecoSV_sameparticle/bbevent_num) + pycolor.END)
    print(pycolor.RED + "MC Charm   / Reco SV : " + str(MCCharm_RecoSV/ccevent_num) \
                      + " Same Chain : " + str(MCCharm_RecoSV_samechain/ccevent_num) + " Same Particle : " + str(MCCharm_RecoSV_sameparticle/ccevent_num) + pycolor.END)
